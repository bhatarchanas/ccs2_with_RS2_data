...
opts = Trollop::options do
	...
	opt :bcfile, "File with the barcodes information, in FASTA format", :type => :string, :short => "-b"
...
opts[:bcfile].nil?     ==false  ? bc_file         = opts[:bcfile]                : abort("Must supply a 'barcode file' which is a FASTA file with barcodes information with '-b'")
...
# Creating dicts to run lima and ccs
lima_hash = {}
ccs_hash = {}
sample_file.each_with_index do |line, index|
	if index == 0
		...
		lima_hash[job_id] = path
		ccs_hash[sample] = [barcode_1, barcode_2, job_id]
	end
end

# Running lima
lima_hash.each do |key, value|
	#puts key, value
	puts "#{smrt_tools_path}lima --different --split-bam --num-threads 32 #{value}/#{key}.subreads.bam #{bc_file} #{out_dir}/#{key}.barcoded.bam"
	`#{smrt_tools_path}lima --different --split-bam --num-threads 32 #{value}/#{key}.subreads.bam #{bc_file} #{out_dir}/#{key}.barcoded.bam`
end
...
ccs_hash.each do |key2, value2|
	...
	if File.file?("#{out_dir}/#{value2[2]}.barcoded.#{correct_bc1_index}--#{correct_bc2_index}.bam")
		file_to_process = "#{out_dir}/#{value2[2]}.barcoded.#{correct_bc1_index}--#{correct_bc2_index}.bam"
		
		if File.exists?("#{out_dir}/#{key2}.ccs.bam")
			if re_run_ccs == "yes"
				`rm -rf #{out_dir}/#{key2}.ccs.bam`

				puts "#{smrt_tools_path}ccs #{file_to_process} #{out_dir}/#{key2}.ccs.bam --numThreads 32 --minPasses #{ccs_passes} --minPredictedAccuracy #{pred_accu}"
				`#{smrt_tools_path}ccs #{file_to_process} #{out_dir}/#{key2}.ccs.bam --numThreads 32 --minPasses #{ccs_passes} --minPredictedAccuracy #{pred_accu}`
